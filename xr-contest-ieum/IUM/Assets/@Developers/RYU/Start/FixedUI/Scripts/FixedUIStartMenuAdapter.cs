...
public sealed class FixedUIStartMenuAdapter : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 원본 씬은 World Space Canvas의 Event Camera가 비어 있어 포인터 입력 경고가 나고,
        // 이벤트 카메라가 없는 월드 캔버스는 uGUI 레이캐스트 대상조차 되지 못한다. 이 캔버스
        // 하나만 연결하면 옵션 등 다른 월드 캔버스의 버튼이 클릭되지 않으므로, 화면 배치에는
        // 손대지 않고 씬의 모든 월드 캔버스에 현재 메인 카메라를 이벤트 카메라로 연결한다.
        var mainCamera = Camera.main;
        if (mainCamera != null)
        {
            foreach (var sceneCanvas in FindObjectsByType<Canvas>(
                         FindObjectsInactive.Include, FindObjectsSortMode.None))
            {
                if (sceneCanvas.renderMode == RenderMode.WorldSpace && sceneCanvas.worldCamera == null)

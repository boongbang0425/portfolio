...
/// Menu: IUM > Dev > Inject PC Player Into Current Scene, IUM > Dev > Create AI Voice Test Scene,
/// IUM > Dev > Create Tutorial Scene, IUM > Dev > Create Prologue Cutscene Scene,
/// IUM > Dev > Create Ending Cutscene Scene.
///
/// 1회성 테스트 씬 생성기(Interaction · Gongpo · Dialogue · Cutscene · Flow · Free Play와
/// Register Test Flow Scenes)는 Play 씬 기반 정식 흐름으로 대체되어 제거했다. 해당 씬 자산은
/// Assets/@Developers/RYU/Scenes/Dev 아래에 그대로 남아 있다.
/// </summary>
public static class DevSceneBuilder
{
    ...
    // 생성기는 제거했지만 튜토리얼 씬의 일시정지 메뉴가 "메인 화면으로" 목적지로 이 이름을 쓴다.
    ...
    /// <summary>
    /// 튜토리얼 대상 도구. 상자에 <see cref="Rigidbody"/>·<see cref="Grabbable"/>을 붙인 일반 도구에
    /// <see cref="ProcessTarget"/> 키와 연속 충돌 판정을 더한 것이다.
    ///

...
public sealed class PlayerInputModule : Module
{
    ...
    void ApplyHandPose(XRHandSide hand)
    {
        ...
        // Device poses are tracking-space, which is the player root's local space. The pose is in
        // metres, so a scene authored at ×20 has to convert it — see Player.WorldUnitsPerMetre.
        // Rotation is unitless and passes through unchanged.
        if (_commands.TryGetHandPose(hand, out var pose))
        {
            anchor.localPosition = pose.position * _player.WorldUnitsPerMetre;

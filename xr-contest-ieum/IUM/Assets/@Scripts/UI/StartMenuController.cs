...
public sealed class StartMenuController : MonoBehaviour
{
    ...
    /// <summary>
    /// 외부 연출이 새 게임 진입을 대신할 때 설정한다(시작 화면 모니터 프롤로그). 덮어쓰기 확인까지는
    /// 이 컨트롤러가 처리하고, 확인이 끝난 뒤의 실제 진입만 위임한다. null이면 기존 GameFlow 경로다.
    /// </summary>
    public Func<Task> NewGameHandler { get; set; }
    ...
    void ApplyPrimaryMenuMode()
    {
        ...
        // 배경만 투명하게 하고 picking을 기본값(Position)으로 두면 UI Toolkit의 PanelRaycaster가
        // 보이지 않는 이 화면을 먼저 히트시켜, 뒤에 있는 월드 캔버스 uGUI 버튼까지 마우스 클릭이
        // 도달하지 않는다. 자식(옵션·확인 패널)은 각자 picking을 유지하므로 열렸을 때는 그대로
        // 동작한다.
        _screen.pickingMode = externalPrimaryMenu ? PickingMode.Ignore : PickingMode.Position;
    ...
    /// <summary>
    /// 외부 확인 UI(시작 화면 모니터의 월드 확인 패널)가 덮어쓰기 확인을 이미 받았을 때의 진입점.
    /// UXML 확인 창(#confirm-panel)을 거치지 않고 곧바로 새 게임을 시작한다.
    /// </summary>
    public void RequestStartNewGameConfirmed() => OnConfirmNewGame();
    ...
    async Task StartNewGameAsync()
    {
        ...
        try
        {
            var handler = NewGameHandler;
            if (handler != null) await handler();
            else await GameFlow.Instance.StartNewGameAsync();
        }
        finally
        {
            // 외부 연출 경로는 씬 전환까지 이어질 수 있으므로 파괴 이후의 UI 접근을 막는다.
            if (this != null)
            {

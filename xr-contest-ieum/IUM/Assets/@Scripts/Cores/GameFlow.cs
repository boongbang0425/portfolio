...
public sealed class GameFlow : Singleton<GameFlow>
{
    ...
    {
        if (!await PrepareNewGameAsync()) return;
        GoTo(ProcessId.Prologue);
    }

    /// <summary>
    /// 새 게임 준비만 수행한다: 진행 초기화와 즉시 저장. 목적지 진입은 하지 않는다.
    ///
    /// 시작 화면이 프롤로그를 자체 연출(중앙 모니터 영상 재생)로 대신할 때 쓴다 — 이 경로는
    /// <see cref="GoTo"/>(Prologue)를 부르지 않으므로 전체화면 프롤로그 컷씬이 중복 재생되지
    /// 않고, 연출이 끝나면 <see cref="CompletePrologueExternallyAsync"/>로 합류한다.
    /// </summary>
    public async Task<bool> PrepareNewGameAsync()
    {
    ...
    /// <summary>
    /// 외부 연출(시작 화면 모니터 영상)이 프롤로그를 끝냈을 때의 완료 처리. 컷씬 경로와 동일하게
    /// 완료를 기록·저장하고 다음 공정으로 진입한다. 건너뛰기도 완료로 기록한다 (F-003 3.5).
    /// </summary>
    public Task<bool> CompletePrologueExternallyAsync() => CompleteQuestAsync(ProcessId.Prologue);
    ...
    /// <summary>
    /// <paramref name="process"/>의 목적지가 씬이면 그 이름을 돌려준다. 컷씬 목적지, 미정의 공정,
    /// 초기화 전에는 null. <see cref="PlaySceneBootstrap"/>이 "저장 진행이 이미 지금 씬을
    /// 가리키는가"를 판별하는 데 쓴다 — 그 경우 재진입 리로드는 불필요하고 해롭다.
    /// </summary>
    public string GetDestinationScene(ProcessId process) =>
        IsReady && _table.TryGet(process, out var entry) && string.IsNullOrWhiteSpace(entry.Cutscene)
            ? entry.Scene
            : null;
    ...
    public bool GoTo(ProcessId process)
    {
        ...
        {
            // 정책: 프롤로그 영상은 시작 화면 모니터(StartMonitorPrologue) 전용이다. 게임플레이 씬
            // (flow.json이 씬 목적지로 쓰는 씬) 위에서 NextProcess가 Prologue로 남아 있으면 —
            // 에디터에서 Play 씬을 직접 실행했거나 프롤로그 미완 저장으로 이어하기한 경우 —
            // 전체화면 컷씬을 겹쳐 재생하지 않고, 모니터 연출 종료와 같은 완료 루틴을 태워
            // 즉시 다음 공정으로 진입한다.
            if (process == ProcessId.Prologue && IsGameplaySceneActive())
            {
                Debug.Log("[Flow] 게임플레이 씬이라 프롤로그 영상을 생략하고 완료 처리합니다.");
                _ = CompletePrologueExternallyAsync();
                return true;
            }
    ...
    /// <summary>
    /// 현재 활성 씬이 flow.json의 씬 목적지(게임플레이 씬) 중 하나인지 확인한다. 씬 이름을
    /// 하드코딩하지 않고 흐름표를 그대로 읽으므로 공정 씬이 늘어도 여기는 손댈 것이 없다.
    /// 컷씬 오버레이는 additive 로드라 활성 씬을 바꾸지 않는다.
    /// </summary>
    bool IsGameplaySceneActive()
    {
        ...
        for (var i = 0; i < _table.Entries.Count; i++)
        {
            ...
            if (entry != null && !string.IsNullOrWhiteSpace(entry.Scene) &&
                string.Equals(entry.Scene, active, StringComparison.Ordinal))
                return true;
        }

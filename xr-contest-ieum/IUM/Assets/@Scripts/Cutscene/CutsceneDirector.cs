...
public sealed class CutsceneDirector : Singleton<CutsceneDirector>, ISceneEventListener
{
    ...
    CutsceneHeadTracking _headTracking;
    Scene _scene;
    ...
    void Update()
    {
        ...
        // 캡션 줄 스킵 Tab(개발용). 컷씬 전체 건너뛰기 Space(유지)와는 별개로 동작하며 줄 하나만
        // 넘긴다. Enter는 강제 진행 디버그 키들과, →는 이동 대체 키와 겹쳐 Tab을 골랐다.
        if (_player != null && _player.IsPlaying &&
            UserInput.HasInstance && UserInput.Instance.GetKeyDown(KeyCode.Tab))
            _player.SkipLine();
    ...
    async Task<bool> PrepareVideoAsync(CutsceneDefinition definition)
    {
        ...
        try
        {
            // 목록 전체를 넘긴다. 단일 영상 항목도 VideoList가 1개짜리 목록으로 돌려주므로 분기가
            // 필요 없다 — definition.Video를 넘기면 분할된 컷씬이 첫 편만 재생하고 끝난다.
            prepared = await EnsureVideoSurface().PrepareAsync(definition.VideoList);
        }
        catch (Exception exception)
        {
        ...
        Debug.LogError(
            $"[Cutscene] '{definition.Id}'의 영상 '{string.Join(", ", definition.VideoList)}'을 열지 못했습니다.");
    ...
    void TakeOverCamera()
    {
        ...
        // 컷씬 카메라에는 추적이 없다. HMD 세션에서 그대로 두면 고개를 돌려도 화면이 고정돼
        // 멀미를 유발하므로, 연출 포즈 위에 HMD 포즈를 합성하는 컴포넌트를 붙인다 (C-1).
        // 데스크톱에서는 null이 돌아와 연출이 종전과 완전히 같다.
        _headTracking = CutsceneHeadTracking.AttachIfDeviceActive(stageCamera);
    ...
    void RestoreCamera()
    {
        // 컷씬 카메라보다 먼저 정리한다. OnDisable에서 연출 포즈를 되돌리므로 씬이 살아 있는
        // 동안 떼어 내야 의미가 있다. 스테이지 카메라가 없어 _suspendedCamera가 비어 있는
        // 경우에도(아래 조기 반환) 이 정리는 반드시 지난다.
        if (_headTracking != null)
        {
            Destroy(_headTracking);
            _headTracking = null;
        }

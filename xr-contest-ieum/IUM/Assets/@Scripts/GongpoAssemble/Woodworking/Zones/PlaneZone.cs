...
public class PlaneZone : WorkZone
{
    ...
    [Tooltip("곡면용(배밀기 등) 구역인지 여부")]
    public bool isCurvedPlaneZone = false;

    protected override void Start()
    {
        base.Start();
        
        // 인스펙터에서 requiredToolType을 직접 CurvedPlane으로 설정했다면 isCurvedPlaneZone을 true로 동기화
        if (requiredToolType == ToolType.CurvedPlane)
        {
            isCurvedPlaneZone = true;
        }
        else
        {
            requiredToolType = isCurvedPlaneZone ? ToolType.CurvedPlane : ToolType.FlatPlane;
        }
    }

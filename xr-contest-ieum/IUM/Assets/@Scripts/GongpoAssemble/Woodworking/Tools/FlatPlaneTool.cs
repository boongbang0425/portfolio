...
public class FlatPlaneTool : MonoBehaviour, IWoodTool
{
    public ToolType GetToolType() => ToolType.FlatPlane;
    ...
    /// 요구하는 존이 한 부재에 섞여 있다(Play 씬 MediumPart 실측: PlaneZoneLeft가 월드 +Z에서
    /// 24.6°, PlaneZoneRight가 158.4°). 단방향 판정이면 그중 절반은 도구를 180° 돌리기 전에는
    ...
    private void Update()
    {
        ...
        if (hasDirection)
        {
            ...
            if (speed >= minCutSpeed)
            {
                ...
                // 면마다 제각각이라 방향 게이트가 실사용을 막았다(PlaneZoneRight 71° 실측). 각도는
                // 판정에 쓰지 않고 품질 점수 계산용으로만 넘긴다.
                validCut = true;
            }
        }
    ...
    private void FinishStroke()
    {
        if (isCutting && currentZone != null)
        {
            ...
            {
                Debug.Log($"[FlatPlaneTool] Stroke failed - distance too short: {currentStrokeDistance:F2}m");
            }
        }

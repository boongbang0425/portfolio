...
public class InkLineTool : MonoBehaviour, IWoodTool
{
    ...
    // ×20 월드 스케일 보정: Play 씬은 플레이어 키가 25유닛이라 1유닛이 인체감 약 7cm다.
    // 기존 고정 반경 0.01f는 인체감 0.7mm라 얇은 먹매김 존을 조준할 수 없었다.
    [Tooltip("먹통 끝(ToolEnd)이 InkLineZone에 닿았다고 볼 반경. ×20 월드 기준 3유닛 = 인체감 약 21cm.")]
    [SerializeField] private float tipCheckRadius = 3f;
    ...
    private void UpdateProjectionVisual()
    {
        ...
        InkLineZone zone = FindZoneAtPosition(checkPos, out Collider zoneCollider);

        if (zone != null)
        {
            // 실제 타격과 동일한 경로로 표면점을 구해 프리뷰와 결과가 어긋나지 않게 한다.
            Vector3 surfacePos = ResolveSurfacePoint(zone, zoneCollider, checkPos);
    ...
    {
        return FindZoneAtPosition(pos, out _);
    }

    // 반경을 넓힌 만큼 여러 존이 동시에 걸릴 수 있으므로, 콜라이더 최근접점 기준으로 가장 가까운 하나만 고른다.
    private InkLineZone FindZoneAtPosition(Vector3 pos, out Collider zoneCollider)
    {
        zoneCollider = null;
        InkLineZone bestZone = null;
        float minSqrDistance = float.MaxValue;

        Collider[] colliders = Physics.OverlapSphere(pos, tipCheckRadius, Physics.AllLayers, QueryTriggerInteraction.Collide);
        foreach (var col in colliders)
        {
            var zone = col.GetComponent<InkLineZone>();
            if (zone == null) continue;

            float sqrDistance = (col.ClosestPoint(pos) - pos).sqrMagnitude;
    ...
    // 판정 반경이 커져 팁이 존 바깥에 있을 수 있으므로, 먼저 존 콜라이더 표면으로 끌어당긴 뒤 목재 표면점을 구한다.
    private Vector3 ResolveSurfacePoint(InkLineZone zone, Collider zoneCollider, Vector3 checkPos)
    {
        Vector3 clampedPos = zoneCollider != null ? zoneCollider.ClosestPoint(checkPos) : checkPos;
        return zone.GetSurfacePoint(clampedPos);
    }
    ...
    private void ProcessToolActivation(bool isCustom)
    {
        ...
        Vector3 checkPos = toolEndTransform != null ? toolEndTransform.position : transform.position;
        InkLineZone targetZone = FindZoneAtPosition(checkPos, out Collider targetCollider);

        if (targetZone == null) return;

        // 점이 찍힐 최종 위치: ToolEnd 위치를 존 콜라이더로 보정한 뒤 표면점 사용
        Vector3 finalPinPos = ResolveSurfacePoint(targetZone, targetCollider, checkPos);
    ...
    public void PinEnd(Vector3 position)
    {
        ...
        if (customCenterGrabbable != null)
        {
            ...
            // 귀환 모듈이 1초 뒤 "홈(먹통 몸체 안)에서 너무 멀다"고 판단해 회수해 버리면 줄을
            // 튕길 수 없게 된다. 핀이 박힌 동안에는 줄 중앙이 제자리이므로 홈을 여기로 옮긴다.
            customCenterGrabbable.RehomeHere();
        }
    }
    ...
    private void SnapString(float tension = 1f)
    {
        if (snapSound != null)
        {
            // PlayClipAtPoint의 기본 감쇠(min 1 / max 500 / Logarithmic)는 미터 전제라 ×12 씬에서
            // 먹줄 튕김이 거의 들리지 않는다. 씬 배율을 반영하는 경로로 바꾼다.
            SceneWorldScale.PlayClipAtPoint(snapSound, stringCenterGrabTransform != null ? stringCenterGrabTransform.position : transform.position);
        }
    ...
    public void ResetTool()
    {
        ...
        if (customCenterGrabbable != null)
        {
            ...
            // PinEnd에서 옮겼던 귀환 기준을 도구 몸체 위치로 되돌린다.
            customCenterGrabbable.RehomeHere();
        }
    }

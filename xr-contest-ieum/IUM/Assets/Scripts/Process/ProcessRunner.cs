...
public sealed class ProcessRunner : MonoBehaviour
{
    ...
    [Tooltip("켜면 저장된 공정의 정의가 이 표에 없을 때 씬 공정으로 폴백하지 않고 대기한다. " +
             "퀘스트 러너와 공존하는 Play 씬 전용.")]
    ...
    /// <summary>현재 단계의 0 기반 순번. 단계 진입 전에는 -1이다. 퀘스트보드의 n/전체 표기용.</summary>
    public int StepIndex => _index;

    /// <summary>현재 공정의 단계 수. 공정이 없으면 0이다.</summary>
    ...
    /// <summary>
    /// 10초 무진전 재안내가 나간 직후. 이음이가 이를 실패 1회로 계상해 세 번 쌓이면 직접
    /// 설명을 허용한다 (F-012 2.4). 공정 자체는 실패 개념이 없으므로 여기서만 신호가 나온다.
    /// </summary>
    ...
    async Task InitializeAsync()
    {
        ...
        if (!BeginProcess(process))
        {
            ...
            // 여기서 멈춰 세우면 그 씬에서 아무것도 할 수 없다. 공존 게이트가 켜져 있으면 다른
            // 러너의 구간이라는 뜻이라 ResolveProcess가 이미 정보 로그를 남겼다.
    ...
    ProcessId ResolveProcess()
    {
        ...
        // Play 씬에는 퀘스트 러너가 함께 있다. 저장 진행이 가리키는 쪽만 시작해야 하므로,
        // 정의 없는 공정은 이 러너의 몫이 아니다 — 폴백하지 않고 Missing으로 대기한다.
        // 튜토리얼 구간에서는 정상 상태라 경고가 아니라 정보로 남긴다.
        if (requireSavedDefinition)
        {
            Debug.Log($"[Process] 저장된 진행 '{saved}'의 제작 공정 정의가 없어 이 러너는 대기합니다.");
            return saved;
        }
    ...
    void TickRetry(float delta)
    {
        ...
        RetryHinted?.Invoke(_current.Data);
    }
    ...
    void OnGUI()
    {
        ...
        // 다른 러너의 구간을 기다리는 중에는 오버레이도 내린다. 튜토리얼 화면에 "정의 없음"
        // 상자가 남아 있으면 오류처럼 보인다.
        if (requireSavedDefinition && _phase == Phase.Missing) return;

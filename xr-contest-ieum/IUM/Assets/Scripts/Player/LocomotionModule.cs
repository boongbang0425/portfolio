...
public sealed class LocomotionModule : Module
{
    ...
    public override void OnFixedUpdate()
    {
        ...
        // A small downward bias keeps the controller grounded on slopes and step edges. The 1 m/s
        // figure is authored in metres, so a ×20 scene needs the same conversion the rest of the
        // player uses — otherwise the bias is 5 cm/s and the controller keeps losing ground contact.
        if (controller.isGrounded && _verticalVelocity < 0f) _verticalVelocity = -_player.WorldUnitsPerMetre;

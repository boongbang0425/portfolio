...
public sealed class DialoguePlayer
{
    ...
    /// <summary>
    /// 재생 중인 줄을 즉시 끝내고 다음 줄로 넘어간다. 개발 중 대사를 빠르게 지나치기 위한 경로다.
    ///
    /// <see cref="Tick"/>과 같은 순서로 이벤트를 낸다(LineFinished → 다음 줄 또는 SequenceFinished).
    /// 다른 점은 줄 사이 <see cref="PlaybackState.Gap"/>을 두지 않는다는 것뿐이다 — 연속으로 눌러
    /// 시퀀스를 훑는 것이 목적이라 간격이 남으면 입력이 씹힌 것처럼 보인다.
    ///
    /// Idle·Loading은 넘길 줄이 아직 없으므로 무시한다. Loading 중 스킵을 큐에 쌓지 않는 것도
    /// 의도적이다 — 준비가 끝나는 시점을 호출자가 알 수 없어 몇 줄이 날아갈지 예측할 수 없다.
    /// </summary>
    public void SkipLine()
    {
        ...
        if (_state == PlaybackState.Speaking)
        {
            // StartLine도 Stop을 부르지만 마지막 줄에서는 거기까지 가지 않는다.
            _source.Stop();

            var index = _lineIndex;
            var isLast = index >= _sequence.Lines.Count - 1;
            LineFinished?.Invoke(CurrentLine);

            // 구독자가 Stop이나 다른 시퀀스 재생으로 응답했으면 이 스킵의 대상은 이미 사라졌다.
            if (_state != PlaybackState.Speaking || _lineIndex != index) return;

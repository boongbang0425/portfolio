...
///
/// 1 튜토리얼 · 2 짧은 부재 · 3 중간 부재 · 4 긴 부재 · 5 조립(도리·공포) ·
...
public sealed class AiProcessDebugSwitcher : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (input.GetKeyDown(KeyCode.Alpha2)) Apply(ProcessId.ShortPart, "짧은 부재에 먹선 긋기");
        else if (input.GetKeyDown(KeyCode.Alpha3)) Apply(ProcessId.MediumPart, "중간 부재 자귀질");
        else if (input.GetKeyDown(KeyCode.Alpha4)) Apply(ProcessId.LongPart, "긴 부재 결구 홈 파기");
        else if (input.GetKeyDown(KeyCode.Alpha5)) Apply(ProcessId.GongpoPuzzle, "도리 걸고 공포 짜 올리기");

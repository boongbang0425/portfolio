...
{
    /// <summary>
    /// 사용 불가 — services still loading, or PTT가 잠긴 구간(컷씬·튜토리얼·일시정지).
    /// 인게임 노장 대사는 더 이상 여기에 해당하지 않는다: 대사 중에도 질문은 받고, 대사를 끊는다.
    /// </summary>
    Unavailable,
...
public sealed class AiConversationManager : Singleton<AiConversationManager>, ISceneEventListener
{
    ...
    /// <summary>
    /// 모델을 못 쓰는 경로(오프라인·LLM 실패)에서 시각 가이드만이라도 띄우기 위한 최소 판정.
    /// 프롬프트의 showGuide가 정상 경로를 맡으므로 여기는 보수적으로 — "보여"나 "어디"가 지시
    /// 대상과 붙은 형태만 본다. "어디"만으로 걸면 "이건 어디에 쓰는 도구야" 같은 원리 질문까지
    /// 가이드를 띄운다.
    /// </summary>
    static readonly string[] GuideRequestKeywords =
    {
        "보여줘", "보여 줘", "보여줄래", "보여 줄래", "보여주라", "보여 주라",
        "보여주세요", "보여 주세요", "보여줘요", "보여 줘요", "보여줄 수",
        "어디야", "어디예요", "어디에요", "어딨어", "어딨는지", "어디 있어", "어디에 있",
        "어디서 해", "어디서 하", "어디부터", "어디를 해", "어디를 하",
        "위치 알려", "위치가 어디", "어디인지 알려", "어디 알려", "어디인지 보여"
    };
    ...
    bool _warnedAboutGuideService;
    bool _answerRequestsGuide;
    bool _inputLocked;
    ...
    /// <summary>
    /// 질문을 받기 시작한 순간부터 답을 마칠 때까지 참. <see cref="InGameDialogue"/>가 이 값을 보고
    /// 노장 대사 시작을 미룬다 — 대화 도중에 대사가 끼어들면 파이프라인이 통째로 취소돼 질문이
    /// 사라졌다.
    /// </summary>
    public bool IsBusy => State is AiConversationState.Listening or AiConversationState.Transcribing
        or AiConversationState.Thinking or AiConversationState.Speaking;
    ...
    /// <summary>
    /// PTT가 잠긴 구간(컷씬·튜토리얼·일시정지) 동안 이음이를 닫는다.
    /// <see cref="PushToTalkLockBridge"/>가 유일한 호출자다 (ISSUE-001).
    /// </summary>
    ...
    public void BeginListening()
    {
        ...
        // 노장 대사가 재생 중이면 PTT가 그것을 끊는다. 대기열은 그대로 남고, 이음이가 답을 마치면
        // InGameDialogue가 이어서 재생한다.
        InterruptDialogue();
    ...
    /// <summary>
    /// 질문이 들어왔으니 재생 중인 노장 대사를 끊는다 (F-011 1.4 완화). 대사 계층을 직접 부르는
    /// 이유는 이 방향에 잠금이 없기 때문이다 — <see cref="InputLockService"/>는 "입력을 막는" 도구라
    /// "이미 재생 중인 대사를 끊는" 요청을 실어 나를 수 없다. <c>TryGetInstance</c>이므로 대사
    /// 시스템이 없는 씬(AiVoiceTest 등)에서는 조용히 지나간다.
    /// </summary>
    static void InterruptDialogue()
    {
        if (InGameDialogue.TryGetInstance(out var dialogue)) dialogue.InterruptForConversation();
    }
    ...
    async Task AnswerAsync(string question, AiSuggestedQuestion suggestion, CancellationToken token)
    {
        ...
        if (!_services.Chat.IsMock && IsOffline)
        {
            ...
            // 준비된 문장은 조작을 설명하지 않으므로, 위치를 물은 질문이면 가이드로 대신 답한다.
            if (WantsGuide(inspection.Text)) TryShowProcessGuide(context);
        ...
        // 안전 검사를 통과해 실제로 발화되는 답에 한해 가이드를 띄운다. 거절되고 버려진 답의
        // showGuide로 화면이 반응하면 말과 화면이 어긋난다.
        if (_answerRequestsGuide) TryShowProcessGuide(context);

        await SpeakAsync(AppendOffTopicSteer(answer), token);
    }

    /// <summary>
    /// 이음이가 조작을 말로 읊는 대신 화면 강조로 안내한다 (F-012 2.4의 직접 안내를 대체).
    /// 서비스가 없는 씬(AiVoiceTest 등)에서는 조용히 넘어가고 경고는 한 번만 남긴다.
    /// </summary>
    void TryShowProcessGuide(AiProcessContext context)
    {
        if (!ProcessGuideService.TryGetInstance(out var guide))
        {
            if (_warnedAboutGuideService) return;
            _warnedAboutGuideService = true;
            Debug.LogWarning("[AI] 씬에 ProcessGuideService가 없어 시각 가이드를 표시하지 못했습니다.");
            return;
        }

        guide.ShowGuide(context?.Process ?? AiProcessContextRegistry.Current.Process);
    }

    /// <summary>모델을 못 쓴 경로의 폴백 판정. 정상 경로는 응답의 showGuide가 맡는다.</summary>
    static bool WantsGuide(string question)
    {
        if (string.IsNullOrWhiteSpace(question)) return false;

        for (var i = 0; i < GuideRequestKeywords.Length; i++)
            if (question.IndexOf(GuideRequestKeywords[i], StringComparison.Ordinal) >= 0)
                return true;
    ...
    async Task<string> GenerateAnswerAsync(string question, AiProcessContext context, CancellationToken token)
    {
        ...
        for (var attempt = 0; attempt < 2; attempt++)
        {
            ...
            if (inspection.IsAllowed)
            {
                ...
                _answerRequestsGuide = parsed.ShowGuide;

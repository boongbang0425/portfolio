...
public sealed class QuestGraphWindow : EditorWindow
{
    ...
    void InsertObjective()
    {
        ...
        var data = new QuestNodeData
        {
            ...
            ControlHint = "플레이어에게 보여 줄 데스크톱 조작 안내를 입력하세요.",
            ControlHintVr = "플레이어에게 보여 줄 VR 조작 안내를 입력하세요.",
    ...
    internal void ShowNodeInspector(QuestNodeView node)
    {
        ...
        AddTextField("조작 힌트 (데스크톱)", node.Data.ControlHint, value =>
        {
            node.Data.ControlHint = value;
            node.RefreshContent();
        }, true);
        AddTextField("조작 힌트 (VR)", node.Data.ControlHintVr, value =>
        {
            node.Data.ControlHintVr = value;
            node.RefreshContent();
        }, true);

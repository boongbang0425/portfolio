...
public sealed class QuestHud : MonoBehaviour
{
    ...
    void Render()
    {
        ...
        // 러너가 다른 러너(제작 공정)의 구간을 기다리는 중이면 카드 자체를 내린다. 제작 공정
        // 화면에 튜토리얼 HUD가 남아 있으면 안 된다.
        if (_boundManager.State == QuestRuntimeState.Dormant)
        {
            Hide();
            return;
        }
    ...
    string CurrentHint()
    {
        // 장치별로 갈린다. HMD가 활성이면 VR 문안, 아니면 데스크톱 문안이다 (VR 문안이 없으면 폴백).
        var hint = _boundManager.CurrentNode?.CurrentControlHint;
        return string.IsNullOrWhiteSpace(hint) ? "강조된 대상과 상호작용하세요." : hint;
    }

    void UpdateSkipHint()
    {
        var key = KeyName(_boundManager.ForceObjectiveKey);

        // 홀드 계측 중: 진행 게이지가 다른 어떤 힌트보다 우선한다.
        if (_boundManager.CanSkipQuest && _boundManager.IsSkipHolding)
        {
            _skip.style.display = DisplayStyle.Flex;
            _skip.text = $"튜토리얼 건너뛰는 중  {BuildHoldGauge(_boundManager.SkipHoldProgress)}";
            return;
        }

        // 튜토리얼 진행 중: 전체 건너뛰기는 제품 기능이라 개발 게이트 없이 항상 안내한다.
        // 개발용 탭 힌트는 문구가 섞이지 않도록 '짧게/길게'로 구분해 병기한다.
        if (_boundManager.CanSkipQuest)
        {
            _skip.style.display = DisplayStyle.Flex;
            var holdHint = $"{key} 길게 눌러 튜토리얼 건너뛰기";
            _skip.text = DeveloperTapHintVisible()
                ? $"{holdHint}  ·  짧게 눌러 현재 목표 건너뛰기 (개발용)"
                : holdHint;
            return;
        }

        // 다른 공정: 기존 개발용 탭 힌트만 유지한다.
        var visible = DeveloperTapHintVisible();
        _skip.style.display = visible ? DisplayStyle.Flex : DisplayStyle.None;
        if (visible)
            _skip.text = $"{key}  ·  현재 목표 건너뛰기 (개발용)";
    }

    bool DeveloperTapHintVisible() =>
        showDeveloperSkipHint && _boundManager.State == QuestRuntimeState.Objective &&
        _boundManager.AllowForceObjective;

    /// <summary>홀드 진행률을 10칸 문자 게이지로 그린다. 별도 UXML 요소 없이 기존 라벨을 쓴다.</summary>
    static string BuildHoldGauge(float progress)
    {
        const int segments = 10;
        var filled = Mathf.Clamp(Mathf.FloorToInt(Mathf.Clamp01(progress) * segments), 0, segments);
